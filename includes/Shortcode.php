...
class Shortcode {
    ...
    public function shortcodeOutput( $atts ) {
        ...
        if ( $glossary && ( array_key_exists( $glossary, $this->settings['glossary']['values'] ) == FALSE )){
            return __( 'Attribute glossary is not correct. Please use either glossary="category" or glossary="tag".', 'rrze-synonym' );
        }
        if ( array_key_exists( $color, $this->settings['color']['values'] ) == FALSE ){
            return __( 'Attribute color is not correct. Please use either \'medfak\', \'natfak\', \'rwfak\', \'philfak\' or \'techfak\'', 'rrze-synonym' );
        }
        ...
        if ( $id && ( !$gutenberg || $gutenberg && $id[0] ) ) {
            ...
            foreach ( $aIDs as $synonymID ){
                $synonymID = trim( $synonymID );
                if ( $synonymID ){
                    $title = get_the_title( $synonymID );
                    $description = str_replace( ']]>', ']]&gt;', apply_filters( 'the_content',  get_post_field( 'post_content', $synonymID ) ) );
                    ...
                    if ( $hide_accordeon ){
                        ...
                        if ( $description) {
                            $accordion .= '[collapse title="' . $title . '" color="' . $color . '" name="ID-' . $synonymID . '"' . $load_open . ']' . $description . '[/collapse]';
                            $schema .= $this->getSchema( $synonymID, $title, $description );
                        }    
                    }        
                    $found = TRUE;
                }
            ...
            $postQuery = array('post_type' => 'synonym', 'post_status' => 'publish', 'numberposts' => -1, 'orderby' => $sort, 'order' => $order, 'suppress_filters' => false);
                    ...
                    foreach( $posts as $post ) {
                        ...
                        if ( $glossary == 'category' && $category ){
                            ...
                            foreach ( $aCats as $slug ){
                                $filter_term = get_term_by( 'slug', $slug, 'synonym_category' );
                                if ( $filter_term ){
                            ...
                            foreach ( $aTags as $slug ){
                                $filter_term = get_term_by( 'slug', $slug, 'synonym_tag' );
                                if ( $filter_term ){
                                    $valid_term_ids[] = $filter_term->term_id;
                                } 
                            }
                        }     
                        $terms = wp_get_post_terms( $post->ID, 'synonym_' . $glossary );
                        if ( $terms ){
                            foreach( $terms as $t ){
        ...
        // 2020-05-12 THIS IS NOT IN USE because f.e. [synonym glossary="category"] led to errors ("TypeError: e.$slides is null slick.min.js" and "TypeError: can't access property "add"" ) as synonyms can have >1 category and so equal sliders would be returned in output which leads to JS errors that avoid accordeons to work properly
        ...
        return '<div class="fau-synonym' . ( $color ? ' ' . $color . ' ' : '' ) . ( isset( $additional_class) ? $additional_class : '' ) . '">' . $content . '</div>';
    }
