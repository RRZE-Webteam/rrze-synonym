...
use function RRZE\Synonym\Config\getOptionName;
use function RRZE\Synonym\Config\getMenuSettings;
use function RRZE\Synonym\Config\getHelpTab;
use function RRZE\Synonym\Config\getSections;
use function RRZE\Synonym\Config\getFields;
...
class Settings {
    ...
    public function showSections() {
        foreach ($this->settingsSections as $section) {
            ...
            if ( $this->currentTab == 'synonymdoms' ){
                $this->domainOutput();
            }            
            echo '</form>';
            echo '</div>';
    ...
    public function callbackLogfile($args) {
        if ( file_exists( $args['default'] ) ) {
            ...
            if ( $lines !== false ) {
                echo '<style> .settings_page_rrze-synonym #synonymlog .form-table th {width:0;}</style><table class="wp-list-table widefat striped"><tbody>';
